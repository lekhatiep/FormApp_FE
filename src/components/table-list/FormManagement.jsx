...
import { NOTIFICATION, ROW_PER_PAGE } from "@constants";
import { URL_SERVER_LOCAL } from "../../constants/common";


function FormManagement(props) {
  const { columns, rows, setRows, handleRefresh } = props ?? {};
  ...
  const handleDeleteFormById = React.useCallback(
    ...
    (id) => {
      const formId = id;
      axios
        .delete(URL_SERVER_LOCAL + `/api/form/deleteDynamicFormById/${formId}` )
        
        .then(() => {
          setIsDeleted(true);
          setRows(rows.filter((item) => item.form_id !== formId));
          setTimeout(() => {
            setIsDeleted(false);
          }, 1500);
        })
